...
def test_register_long_username(client):
    ...
    assert response.status_code == 200
    assert b"Field cannot be longer than 80 characters" in response.data or b"Invalid" in response.data or b"Register" in response.data or b"between" in response.data
