...
## 2024-08-14 - [Fix] Unbounded In-Memory Data Structures (DoS Risk)
**Vulnerability:** The application used unbounded dictionaries (`login_attempts` in authentication and `_rate_limit_cache` in `ai_service.py`) for rate-limiting logic. Because the dictionaries never constrained their maximum size or aggressively pruned older entries, an attacker could continuously make requests from spoofed IPs or user IDs, causing the dictionaries to grow indefinitely, ultimately exhausting server memory and causing a Denial of Service (DoS).
**Learning:** Any in-memory state tracking (especially rate limiters or caches keyed by user input like IPs) must have strict upper bounds. If an upper bound is reached, the structure must be pruned or cleared to prevent out-of-memory crashes.
**Prevention:** Always bound dictionaries used for caching or rate limiting. For simple apps, `if len(cache) > MAX_SIZE: cache.clear()` is a quick defense. For robust applications, use a dedicated key-value store with TTLs like Redis.
