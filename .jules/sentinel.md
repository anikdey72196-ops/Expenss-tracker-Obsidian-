...
## 2026-08-15 - [Fix] In-Memory Rate Limiting and DoS Prevention on Login
**Vulnerability:** The `/login` endpoints in both web and API pathways lacked rate limiting, exposing the application to brute-force and credential stuffing attacks. Additionally, naively tracking rate limits in an unbounded in-memory dictionary can lead to memory exhaustion (Denial of Service).
**Learning:** Security mechanisms like rate limiting must themselves be bounded to prevent them from becoming an attack vector (e.g. memory DoS via spoofed IPs). Global test state must also be isolated to prevent intermittent test failures.
**Prevention:** Implement bounded rate limiting dictionaries with explicit pruning mechanics. Clear global rate limit state inside testing fixtures.
