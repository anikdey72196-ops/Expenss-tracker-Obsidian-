...
## 2024-08-05 - [Fix] Missing Rate Limiting on Login Endpoints
**Vulnerability:** The application was missing rate limiting on both web and API `/login` endpoints. This allows attackers to perform brute-force or credential stuffing attacks, making it easier to guess user passwords.
**Learning:** Authentication endpoints must have rate limiting implemented to mitigate brute force attacks and protect user accounts. We should implement rate limits for both the Flask web routes and the Flask API routes, keeping track of attempts by IP. Global state must be explicitly cleared within the pytest `client` fixture to prevent state leakage and false failures across test cases, per memory constraints.
**Prevention:** Implement rate limiting using an in-memory dictionary or a proper rate limiting tool to restrict the number of login requests from a specific IP within a given time frame (e.g., max 5 attempts per minute). Always clear the dictionary in testing fixtures.
