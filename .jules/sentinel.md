...
## 2026-08-19 - [Fix] In-Memory Rate Limiting Dictionary Bounds (DoS Prevention)
**Vulnerability:** The application was using Python dictionaries (`{}`) to track rate-limiting limits for authentication endpoints and AI services. However, these dictionaries were strictly additive without any maximum size boundaries or pruning mechanisms. An attacker continuously generating requests with unique IP addresses could easily crash the application by filling memory until an Out-of-Memory (OOM) error causes a Denial of Service (DoS).
**Learning:** Any application state held in-memory directly tied to user requests, like rate limit IP-addresses or caching keys, must have a strict upper-bound limit.
**Prevention:** In addition to time-based rate limit rules, always implement length checking boundaries on in-memory collections (`len(cache) > MAX_SIZE`). When it exceeds the limit, selectively prune expired keys, and if flooded during a short period, aggressively flush the entire cache to prioritize application survival.
