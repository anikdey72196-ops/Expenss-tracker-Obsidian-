...
## 2024-08-08 - [Fix] Inconsistent API Validation Logic & Potential DoS
**Vulnerability:** The `/signup` API endpoint in `auth.py` had confusing and heavily duplicated validation logic for usernames and passwords that set conflicting bounds on length limits. A duplicate/broken validation line in the `/login` endpoint incorrectly checked for password lengths up to 128 characters instead of 72.
**Learning:** Having redundant length validations with different string limits in the same code path increases maintenance overhead and leads to inconsistent validation. The 72-character limit on passwords is crucial to prevent bcrypt exhaustion attacks.
**Prevention:** Validation checks should be dry and explicit. Only have one source of truth for constraints (like max username 80 characters, and max password 72 characters) and enforce them before saving strings to the database.
