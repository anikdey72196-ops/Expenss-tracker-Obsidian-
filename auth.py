...
def signup():
    ...
    if not isinstance(data['username'], str) or len(data['username']) > 80 or len(data['username']) < 3:
        return jsonify({"error": "Username must be between 3 and 80 characters."}), 400

    if not isinstance(data['password'], str) or len(data['password']) > 72 or len(data['password']) < 8:
